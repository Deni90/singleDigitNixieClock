...
LedInfo
NixieClockInterface::OnGetBacklightData() const {
    return ledController.GetLedInfo();
}
bool
NixieClockInterface::OnSetBacklightState(uint8_t state) {
...
void
NixieClockInterface::OnSetBacklightColor(uint8_t r, uint8_t g, uint8_t b) {
    if (ledController.GetLedInfo().GetR() == r &&
        ledController.GetLedInfo().GetG() == g &&
        ledController.GetLedInfo().GetB() == b) {
...
void
NixieClockInterface::OnSetCurrentTime(uint16_t year, uint8_t month,
